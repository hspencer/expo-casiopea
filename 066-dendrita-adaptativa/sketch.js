// Dendrita adaptativa (grafo dirigido por fuerzas, 5000 nodos)
// Nace un nodo en el centro y de él brotan aristas hacia nuevos nodos que se
// ramifican formando una dendrita. El grafo es físico: los nodos se repelen
// entre sí y las aristas son resortes. Cuando el borde se satura (nodos
// apretados con distancia < UMBRAL_APRETADO), ~100 de ellos se reubican al
// centro, se dejan asentar 1 seg, se conectan sólo a sus 2 vecinos más
// cercanos y el crecimiento continúa. Al completarse los 5000 nodos la imagen
// queda congelada y recorrible. ESPACIO reinicia.
//
// La repulsión usa partición espacial (grilla) para mantener O(n) en vez de
// O(n²), necesario a esta escala.
//
// Fondo negro, nodos blancos como estrellas, aristas gris claro transparentes.
...
// --- Paleta -------------------------------------------------------------
const FONDO = 0;             // negro
const NODO_COLOR = 255;      // blanco
const EDGE_COLOR = 210;      // gris claro
const EDGE_ALFA = 55;        // muy transparente
...
// --- Reubicación al centro (algoritmo adaptativo) -----------------------
const UMBRAL_BORDE = 0.85;
const UMBRAL_APRETADO = REST * 1.5;
const MIN_RELOCAR = 5;
const MAX_RELOCAR = 100;
...
// Busca nodos en la zona del borde que tienen algún vecino del borde a
// distancia < UMBRAL_APRETADO. Devuelve hasta MAX_RELOCAR índices elegidos
// al azar del conjunto apretado.
...
// reubicarAlCentro
// Distribuye los nodos en anillos concéntricos y resetea velocidades.
function reubicarAlCentro(indices) {
  const n = indices.length;
  for (let k = 0; k < n; k++) {
    const i = indices[k];
    const ang = (k / n) * TWO_PI + random(-0.4, 0.4);
    const r = REST * (2 + Math.floor(k * 5 / n));
...
// conectarReubicados
// Tras el asentado, conecta cada nodo reubicado sólo con sus
// N_VECINOS_RECONECTAR vecinos más cercanos aún no enlazados.
function conectarReubicados(indices) {
  for (const i of indices) {
    ...
    for (let j = 0; j < nodos.length; j++) {
      ...
      cerca.push([Math.hypot(a.x - nodos[j].x, a.y - nodos[j].y), j]);
    }
...
// fisica
// Un sub-paso de simulación. La repulsión usa una grilla espacial (O(n))
// en vez de comparar todos los pares (O(n²)), esencial para 5000 nodos.
function fisica() {
  ...
  // Resortes en las aristas (ley de Hooke)
  ...
  // Integración con topes, amortiguación y confinamiento al círculo
  for (let i = 1; i < n; i++) {
    const p = nodos[i];
...
// draw
// Máquina de fases:
// 'creciendo'  → agrega nodos; detecta saturación y dispara asentado.
// 'asentando'  → sólo física 1 seg; luego reconecta y vuelve a crecer.
// 'congelado'  → noLoop(); imagen estática recorrible.
function draw() {
  if (fase === 'creciendo') {
...
// dibujar
// Fondo negro, aristas gris claro muy transparentes, nodos blancos como
// estrellas. El nodo central aparece un poco mayor.
function dibujar() {
  background(FONDO);
  ...
  // Aristas: gris claro, muy transparentes
  ...
  // Nodo central ligeramente mayor
